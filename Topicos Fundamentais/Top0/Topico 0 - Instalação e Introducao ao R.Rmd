...
- O **R** é voltado para o tal do __datascience__ . Ou seja, você para usa-lo no dia a dia você precisa aprender menos conceitos ligados ao desenvolvimento de software do que para outras linguagens. Isso por exemplo se expressa no **R** o índice começar no `1`, enquanto a maioria das linguagens começa no `0`. Isso na hora de pensar uma matriz pode fazer muita diferença.

- Seguindo do ponto anterior, várias funções de algebra linear são próprias do **R**. Não é necessario carregar nenhum pacote ou algo do gênero para inverter uma matriz.
...
Vá em https://cran.r-project.org/ e siga o procedimento da imagem abaixo (a versão provavelmnete será diferente da figura):

![](gifs/gif_intro_download_R.webp)

A versão da tela é a 3.6.3, mas pode baixar qualquer uma que aparecer.^[4] Depois siga os procedimentos abaixo para a instalação:
...
**Importante: Se por algum motivo você não conseguir instalar o Rtools, não tem problema para os exercicios que iremos fazer neste curso. Porém algumas funcionalidades de certos pacotes do R pressupõem o Rtools.**
...
## [O tidyverse: Introdução](#tidyverseintro)
...
[^4]: Imagem retirada do tutorial da [Datacamp](https://www.datacamp.com/community/tutorials/installing-R-windows-mac-ubuntu).
[^5]: A mais relacionada com `subsetting`
