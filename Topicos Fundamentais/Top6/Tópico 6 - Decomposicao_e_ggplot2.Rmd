...
-   Pode ser através do **nome** da cor, tal como "red", "blue", etc. Lembrando que a lista de cores possíveis pode ser obtida por `colours()`.
...
Era de se esperar uma correlação positiva e próxima de uma reta com inclunação de 45 graus, dado que os multiplicadores refletem informações estruturais que demoram mais tempo para apresentarem mudanças significativas. Vamos agora fazer outro exercício, que é comparar o multiplicador do emprego com o do VA e o dos Salários. Antes disso é necessário criar ps multiplicadores do VA e W, com uam definição semelhante à que foi usada para o emprego:

```{r, eval=F, include=F}
VADF <- tibble::tibble("y2010" = matrizes2010$Psi,
                        "y2015" = matrizes2015$Psi)
...
```{r coment, eval=F}
Buscar exemplo de alguma série temporal

possivelmente taxa de crescimento do pib e de outro componente (taxa de investimento)
```
...
```{r, eval=F}
comparar os multiplicadores do emprego em 2010 e 2015.
```
