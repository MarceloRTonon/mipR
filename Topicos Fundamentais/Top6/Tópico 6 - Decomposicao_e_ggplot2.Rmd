...
A primeira decomposição polar é:
$$\frac{\Psi_1}{\Psi0} = \left(\frac{\lambda^1 L^1 f^1}{\lambda^0 L^1 f^1}\right)
...
### Decomposição de "Matrizes Internas"

Essa parte é baseada no capítulo 13 do Miller e Blair(2009, p. 603-604). Nela os autores chamam isso de decomposição da matriz de coeficientes diretos. No entanto esse ferramental também pode servir para decompor gastos endógenos (como o consumo induzido). Por isso, por falta de definição melhor, escolhemos usar o termo _matriz interna_^[Esse nome foi criado por nós, logo pode ser mudado. Queremos apenas expressar aqui a relação.], que no fim se refere à uma matriz $M$ qualquer de matriz interna em relação à $N$ se ela define $N$ da seguinte forma: $$N = [I-M]^{-1}$$.
...
$$L^1 - I = L^1A^1$$
E depois **pós-multiplicando** por $L^0$ teremos:

$L^1L^0 - L^0 = L^1A^1L^0$

Se fizermos uma **pré-multiplicação** de $L^0$ por $(I-A^0)$  teremos:

$$(I-A^0)L^0 = I = L^0 - A^0L^0$$
Podemos então (de forma similar ao feito para $L^1$) rearranjar a equação de $I = L^0 - A^0L^0$ e **pré-multiplicar** por $L^1$, de modo que teremos:

$$L^0 - I = A^0L^0 \Rightarrow L^1L^0 - L^1 = L^1A^0L^0$$

Subtraindo então $L^1L^0 - L^0$ por $L^1L^0-L^1$ teremos:

$$\left(L^1L^0 -L^0\right) - \left(L^1L^0 -L^1 \right) = L^1A^1L^0 - L^1A^0L^0$$
Rearranjando cada um dos lados, e lembrando das propriedades de matriz, teremos que:
...
Se fizermos as distribuição dos valores internos aos colchetes $[]$, teremos que:
...
Sendo a primeira parte, $\left(\tfrac{1}{2}\right)\left[L^1 \Delta D \left(B_n^0 + B_n^1 \right) L^0\right]$ referente os efeitos da variação de $D$ sobre $L$ e o segundo $\left(\tfrac{1}{2}\right)\left[L^1\left(D^0 + D^1\right) \Delta B_n L^0\right]$ referente os efeitos da variação de $B_n$ em $L$.
...
Desse modo, podemos combinar de inúmeras formas decomposições aditivas com as decomposições de matrizes internas. É possível realizar decomposições da Matriz de Miyazawa $ c_W\omega'$ que esitver dentro de um multiplicador $X = [I-A-c_w \omega'] f_o$^[Ver Krepsky(2019) para tanto]. Existe também outras possibilidades, para decompor uma matriz. Vejamos:
...
Podemos então fazer com que $A_n = A_{n1} + A_{n2}$. Como se trata de uma **soma**, a decomposição de $\Delta A_n$ é simplemente:
$$\Delta A_n = \Delta A_{n1} + \Delta A_{n2}$$
```{r}
...
Um gráfico muito usado no pacote `ggplot2` é o `geom_poit()`. Ele permite
...
Dietzenbacher, E. and B. Los (2000) Structural decomposition with dependent determinants, Economic Systems Research, 12, pp. 498-514.
