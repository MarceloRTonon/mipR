...
#### Q.2.2 - No exercicio do Tópico 2 foi os dados da niot para a Italia, cujo código era `ITA`. Sabendo que todos os arquivos de outros paises da niot seguem o mesmo padrão, crie um vetor `niotQ2.2` usando os valores de `paisesQ2.1` para criar um vetor com os links dos países de `paisesQ2.1`.

```{r, include=FALSE}
wd <- getwd()
niotQ2.2 <- paste0(paisesQ2.1,"_NIOT_nov16.xlsx")
```
...
#### Q.2.5 - Extraia apenas o primeiro valor de `niotQ2.2` e use-o para importar a base de dados a partir desse arquivo tal qual foi feito para `niotITA` no inicio do exercicio do Tópico 3. Aloque esse tibble como o primeiro elemento de `niotQ2`
...
niotQ2 <- list()
niotQ2[[1]] <- read_xlsx(niotQ2.2[1], sheet = 2)
...
# Importando a NIOT para o R:
niotQ2.6 <- list()
niotQ2.6[[i]] <- read_xlsx(niotQ2.2[i], sheet = 2)
```
...
for(i in 1:length(niotQ2.2)){

# Importando a NIOT para o R:
niotQ2[[i]] <- read_xlsx(niotQ2.2[i], sheet = 2)
}
toc()
```
