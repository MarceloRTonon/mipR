...
Nesta seção, serão explicadas o que é um loop e porquê não fazer manualmente tarefas que envolvem repetições. Além disso, iremos contextualizar como funciona o loop no R, bem como a forma de fazê-lo e algums nuances sobre ele. 

Por fim, vamos também falar do que é uma função vetorizada e apresentar a família `map` do pacote `purrr`.
...
Uma das grandes vantagens de se programar é a automação do processo. Assim, suponha que você quer calcular os produtos da multiplicação de `umObjeto` qualquer pelos valores de 1 a 3. Assim sendo, você pode fazer:
...
E poderíamos fazer assim por diante. Quando falamos em automatizar o seu código, falamos em dar de uma vez ao computador um conjunto de informações para que ele tenha a capacidade de reproduzir um código diversas vezes para cada elemento deste conjunto. No exemplo acima, se usarmos o Loop, ele irá rodar o seu código do inicio ao fim para cada valor do objeto `ano` que você determinar.
...
Pelas caracteristicas de linguagens como a do R, há uma penalização em sua performance ao usar loopings. Por conta disso, o mais comum é, a partir de um certo momento, abandonar o uso do loop em detrimento da programação funcional. No caso do R, aconselhamos que seja pelo pacote `purrr`. Não falarei aqui o porque disso, mas vale deixar claro que o `for` não é a linha de chegada quando se trata de automatizar os seus códigos.
...
Uma função vetorizada é uma função que opera sobre um conjunto de valores dentro de um vetor de uma só vez. Neste caso, falamos aqui de vetor enquanto estrutura de dados como um todo, seja um vetor atômico, seja uma lista. Assim sendo temos alguns exemplos de funções vetorizadas.
...
- A depender do tamanho do vetor (em especial listas extremamente recursivas), a forma vetorial pode travar (pois o computador não tem memoria RAM para tanto). Vale notar que, no geral, o próprio loop também não vai funcionar muito bem nesses momentos. É uma questão de hardware. Porém, é apenas para objetos que estiverem na casa dos gigabytes (que não costuma ser o caso de dados de insumo-produto)!

## Introduzindo `map`, `map2` e `pmap`

Vale notar certa coisa sobre o uso das operações lógicas e matemáticas enquanto funções vetorizadas: eles não se aplicam a listas. Além disso, para o caso de análises usando Matrizes Insumo-Produto, muitas vezes precisamos usar operadores (como o próprio `%*%`) para várias matrizes dentro de uma lista com grande recursividade. Assim sendo, o pacote `purrr` oferece uma forma especial para tratarmos de listas, em especial com a família `map()`.
...
Porém, pode ainda estar confuso como funciona uma função `purrr::map()`. Pense assim: a função `map()` pega uma função e aplica, de forma vetorizada, sobre todos os elementos de uma lista. Assim sendo, temos que `purrr::map` é como um comando `for`, porém mais prático e mais fácil. Veja abaixo:
...
No looping acima teríamos que o primeiro (segundo) elemento de `lista_mat`, a matrix `A` (`B`), seria multiplicado pelo primeiro (segundo) elemento da `lista_escalares`, `2` (`3`). Podemos no entanto, usar uma variante de `purrr::map` para tanto, a função `purrr::map2`. Veja abaixo:
...
## Recursividade na família `map`

Tratamos nos últimos casos, de uma lista com recursividade 1. Ou seja, não havia lista dentro de uma lista neste caso. Porém, vamos criar a lista `lista_recursiva`:
...
É importante notar que a forma como fizemos agora, requer que tanto `lista_recursiva` quanto `lista_escalares2` tenham a mesma recursividade. Há, no entanto, uma forma usar `lista_escalares` ao invés de `lista_escalares2`. Essa forma é combinando `map` com `map2`. Veja abaixo as duas formas de escrever isso:
...
Suponha agora que você quer criar uma lista com os elementos da base de dados `CO2`. Porém, você quer separar ela por `Treatment` que ela tem.^[**Importante:** o exemplo dado aqui poderia ser feito usando várias funções do `dplyr` porém queremos deixar claro como criar uma lista via `purrr`] Vamos criar então uma lista com cada valor `Treatment`:
