...
```{r}
Q4.Leontief <- map(A_matrixes, function(x) solve(diag(nrow(x)) - x))
```
...
# crossprod é mais rápido do que %*%!
x_VBP <- map2(Q4.Leontief, finalDemand, crossprod)
```

#### Q.4.5 - Crie uma lista `l_matrixes`, com as matrizes de impacto do trabalho $ \hat{l} %*% [I-A]^-1$. 
...
```{r}
l_matrixes <- map2(l_vetor, Q4.Leontief, function(x,y) crossprod(diag(c(x)), y))

create_lA <- function(x,y) crossprod(diag(c(x)), solve(diag(nrow(y))- y)) 
...
```{r}
L_setor <- map2(l_vetor, finalDemand, crossprod)
```

##### B. Faça a questão `Q.4.6` usando `l_vetor`, `Q4.Leontief` e `finalDemand`

```{r}
L_setor <- list(l_vetor, Q4.Leontief, finalDemand) %>%
  pmap(function(x,y,z) crossprod(diag(c(x)), crossprod(y, z)))
```
