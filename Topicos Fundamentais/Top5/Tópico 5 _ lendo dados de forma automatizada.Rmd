...
Há também a possibilidade de você combinar strings usando a função `str_c()`. Além dos vetores que você deseja combinar, há também a possibilidade de colocar um separador entre as strings (sep) e também `collapse` em que para valores diferentes de NULL cria-se uma única string com todos os elementos.
...
# FALTA AINDA FALAR DE str_subset
...
Vamos então agora fazer a leitura da aba "VA" em todos esses arquivos. Essa aba não consta em todos os arquivos listados, somente nas tab2 e tab4, então:

```{r}
TRU_tab2_tab4 <- str_subset(TRUfiles, pattern = c("tab2","tab4"))#VERIFICAR ISSO!
```


Uma maneira simples e rápido de fazer isso é usando a função `map` que aprendemos nas aulas anteriores:
...
Agora que separamos as informações podemos, por exemplo, criar um arquivo compactado com as informações já trabalhadas.

```{r, eval=F, include=F}

FALAR AQUI SOBRE A CRIAÇÂO DE ARQUIVOS COMPACTADOS
