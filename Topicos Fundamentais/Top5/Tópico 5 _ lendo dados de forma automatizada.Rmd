...
Uma função que já foi abordada nos exercícios, mas que ainda não fio devidamente apresentada é a `download.file()`. Como já visto ela é particulamente útil para a automação e reprodução de um código, realizando o download automático de algum arquivo a partir de sua url na internet. Simplificando para os argumenta mais comuns da função, ela é estruturada da seguinte forma:

```{r,eval=F}
download.file(url, destfile, method, mode = "w", headers = NULL)
```
...
Existe também a possibilidade de se descompactar automaticamente arquivos compactados.

```{r}
Encoding(TRU_2010_2017) <- "UTF-8"
unzip(TRU_2010_2017,exdir = "TRU")
```
...
Tal como no nosso exemplo em que existe uma quantidade razoável de arquivos a sereim lidos, casp fosse realizada a leitura individual de cada arquivo poderíamos ter um código longo e repetitivo. Uma alternativa é realizar a leitura silmultânea de várias arquivos, usando a função `list.files()`.
