...
Vamos agora apresentar o que seria uma "string" e como isso se difere de um "character". Em programação de modo geral, um "character" é somente o caracter isolado, por exemplo, a letra "a", enquanto uma "string" é um conjunto de caracteres que podem, por exemplo formar uma palavra ou uma frase. Dentro de uma string podemos ter diversos caracteres (inclusive numéricos ou símbolos)


No caso específico do R, não há diferenciação entre `character` e `string`, sendo todos objetos desse tipo sendo tratados como `character`. 
