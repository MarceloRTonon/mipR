...
#### Q.1.7 Faça uma inversa de Leontief a partir de `dfQ1.5`. Nomeie-a `dfQ1.7` e faça coerce-a em um `data.frame`.
...
**Dica:** Mude a configuração do argumento `check.attributes` de uma das .
...
#### Q.3.3 Nomeie as linhas de `dfQ3.1` com `cod67`.

```{r, eval = F, include = F}
rownames(dfQ3.1) <- cod67
```

#### Q.3.4 Exporte ``dfQ3.1` no formato csv separado por **virgulas** e decimal com `.`. Chame o arquivo de `dfQ31.csv`

```{r, eval = FALSE, include = FALSE}
write.csv(dfQ3.1, file = "dfQ31.csv")
```

#### Q.3.5 Exporte ``dfQ3.1` novamente só que agora no formato csv separado por **ponto e virgula** e decimal por **virgulas**. Chame o arquivo de `dfQ31b.csv`

```{r, eval=F, include=F}
write.csv2(dfQ3.1, file = "dfQ31b.csv")
```

#### D.3.1 - **Desafio:** usando a função `write.xlsx` do pacote `openxlsx`, crie um arquivo 