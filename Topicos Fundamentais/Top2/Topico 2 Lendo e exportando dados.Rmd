...
O foco deste tópico é apresentar as diferentes funções responsáveis pela leitura dos dados e também a sua exportação de dados. Repare que neste caso específico quando falamos aqui de exportação de dados estamos nos referindo a salvar arquivos no Disco Rígido Local que contenham os dados produzidos por nós no R. Existem outras formas de exportar dados, como produzir tabelas para *LaTeX*^[Ver pacote `tabulizer`] e Word^[Veja esse [guia](https://sejdemyr.github.io/r-tutorials/basics/tables-in-r/)] e dar `Commit` e `Push` via `Git`^[Existe o livro online [*Happy Git and GitHub for the useR*](https://happygitwithr.com/) ou o [material em português da Beatriz Milz em um evento da  `#Rladies`](https://beatrizmilz.github.io/RLadies-Git-RStudio-2019/#1). Temos também um material nosso sobre o tema, que foi baseado no Happy Git, o qual vocês podem baixar [aqui](https://raw.githubusercontent.com/MarceloRTonon/mipR/master/Topicos%20Suplementares/Git/intro_git.html), dando o ctrl+s na página.].
...
Uma vantagem do `.Rdata` é que ele te permite salvar vários objetos de uma só vez. Existe uma variação da função `save` inclusive que diz para salvar todos os objetos presentes no seu `Global Environment`^[Present na aba `Environment`, que na configuração padrão fica no lado direito superior do Rstudio.]: `save.image`. Veja abaixo, o exemplo baseado no vindo da `R documentation` da função `save`:
