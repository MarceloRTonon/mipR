...
### Questões dos Operadores Booleanos

Suponha que você conhece 5 pessoas. E que vocẽ então listou 4 caracteristicas binárias^[Ou seja, cujas respostas são sim ou não. 
Pergunta Binária: Fulano é loiro?
Pergunta não binária: Qual a cor do cabelo de Fulano?]
~quaisquer e atribuiu um valor lógico de verdadeiro ou falso se elas tem ou não aquela caracteristica.. Não vamos dar nomes as pessoas nem caracteristicas, apenas vamos chamar tudo pelo número. Abaixo vamos criar um vetor para cada pessoa, colocando `TRUE` ou `FALSE` se ela tem ou não a caracteristica (`Car_`).
...
**IMPORTANTE:** Nas próximas questões queremos apenas os resultados com os valores lógicos. Não precisa fazer um código tentando indicar automaticamente um valor `character` para isso. Um resultado como `r  c(FALSE, TRUE, FALSE, TRUE)`, ou mesmo `r FALSE` quando for o caso, está de ótimo tamanho. Se tiver como  `r  c(FALSE, TRUE, FALSE, TRUE)` já é entendido por exemplo que para `Car_1` a resposta é não, para `Car_2` sim, `Car_3` não e `Car_4` sim. Não precisa tentar falar o nome da caracteristica como `r c("Car_2", "Car_4")`. Que fique claro: **Não precisa**, _mas se quiser pode._

**PRESTE ATENÇÃO** quando queremos resultados específicos sobre cada uma das `caracterististicas` ou quando queremos comparação geral sobre as `pessoas`. Se você não entendeu essa dica, volte para o material na parte da tabela dos operadores booleanos.
...
#### Q.1.9
#### Q.1.8 Crie uma `anti_pessoa_3` que possuí todas 
