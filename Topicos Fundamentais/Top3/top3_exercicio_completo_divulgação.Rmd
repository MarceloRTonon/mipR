...
Ou seja, cujas respostas são sim ou não. 
**Pergunta Binária:** _Fulano é loiro?_ **Pergunta não binária:** _Qual a cor do cabelo de Fulano?_
]
quaisquer e atribuiu um valor lógico de verdadeiro ou falso se elas tem ou não aquela caracteristica. Abaixo estão os valores nos quais vocẽ chegou no final.
...
Dica: Use dois operadores booleanos para tanto e então compare o resultado com `identical`.


#### Q.1.10 Considere `pessoa_1` e `pessoa_2` como uma dupla que se complementa. Teste para saber se a dupla possuí, em qualquer um de seus membros,  todas as caracteristicas listadas. 

Detalhe: o vetor da resposta correta tem comprimento `1`.

#### Q.1.11 Para qualquer combinação de pessoas listadas o operador `&&` sempre retornará o mesmo resultado. Olhe para os dados e diga: por que não pode ser diferente? Quais características nos vetores seriam necessárias para usar isso. Por que isso não é o caso de `||`?
...
#### Desafio D.2.2 Seguindo o desafio anterior, suponha que você quer criar uma função para saber se o vetor como um todo passa nesse teste (ou seja, se ele retorna tudo como `TRUE`). Como você faria isso? Por que não dá para vocẽ usar 

**Detalhe:** O código que vocẽ vai fazer aqui vai ser ineficiente mesmo. É só para você tentar entender a questão como um todo.

#### Q.1.12 - Explique as diferenças entre `&`, `&&`, `|` e `||`.
...
total_output_diag <- diag(1/as.double(total_output[,5:60]))
