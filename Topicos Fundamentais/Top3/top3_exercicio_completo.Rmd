...
```{r, include=FALSE}
dfQ4.7 <- americadoSul %>% anti_join(edicoes, by = c("Países" = "Vice"))
```
...
E seja a definição do *Argument* `row.names` da função `data.frame` (veja o R Documentation) a seguinte:
...
##### Q.5.11.1
```{r, include = TRUE, eval = FALSE}
dfQ5.11.1 <- data.frame(dfQ5.11, row.names = t(df.nomes["Nomes"]))
```
...
#### Q.6.1 - Revise a [Table 7.1](https://rstudio-education.github.io/hopr/modify.html#tab:logop) do **HOPR** para deixar claro como funciona cada operador lógico.

#### Q.6.2 - Faça o mesmo para os Operadores Booleanos na [Table 7.2](https://rstudio-education.github.io/hopr/modify.html#tab:boole).

#### Q.6.3 - Atribua valores arbitrários para "a", "b" e "c" na Table 7.1 e explique com suas palavras o resultado de cada teste.

#### Q.6.4 - Explique as diferenças entre `&`, `&&`, `|` e `||`
...
#### Q.6.5 - Teste a condição (`x > 5 & < 10`). Por que ela não funciona? Como você resolveria?
...
#### Q.7.1 - Quais as diferenças entre usar if, else e ifelse? Quais as melhores situações para usar cada uma delas?
...
#### Desafio 7.1 - Crie uma função para saber se um número inteiro é par ou ímpar. **Dica** pensar no resto da divisão (Operador %% no R).
...
#### Q.7.2 - No material de Algebra Linear, discutimos que para uma matriz ser invertível é necessário que ela atenda à algumas condições. Escreva uma função que diga se as matrizes abaixo são invertíveis ou não, e diga se as matrizes abaixo tem inversa.
...
tem_inversa <- function(x){
  ifelse(det(x) != 0 & nrow(x) == ncol(x), "Tem Inversa", "Não tem Inversa")
}
```
...
#### Q.8.1 - Visualize `niotITA` e perceba que existem valores `NA` por conta do cabeçalho da base de dados. Ajuste o cabeçalho de acordo com o conhecimento adquirido no tópico anterior, criando uma lista com o nome das variáveis.

```{r, include=FALSE}
nomes_colunas <- c(colnames(niotITA[,1:4]), niotITA[1,5:67])
```


#### Q.8.2 - Elimine a linha que contém `NA` e renomeie as colunas de `niotITA`. Salve como `niotITA_8.2`.

```{r, include=FALSE}
niotITA_8.2 <- niotITA[-is.na(niotITA),] %>% type.convert()
colnames(niotITA_8.2) <- nomes_colunas
```

#### Q.8.3 - Extraia da sua base valores somente para o ano de 2008. Salve como `niotITA_8.3`

```{r, include=FALSE}
niotITA_8.3 <- niotITA_8.2[niotITA_8.2$Year == "2008",]
```

#### Q.8.4 - Agora pegue apenas os valores que tem origem doméstica e salve no arquivo `nitoITA_8.4`.

```{r, include=FALSE}
niotITA_8.4 <- niotITA_8.3[niotITA_8.3$Origin == "Domestic",]
```

#### Q.8.5 - Caso você quisesse fazer as duas últimas questões em só uma linha de comando, como você faria? (**Dica**: lembrar dos operadores Booleanos). Não esqueça de comparar com a base gerada na questão anterior e se elas são idênticas.

```{r, include=FALSE}
niotITA_8.5 <- niotITA_8.2[niotITA_8.2$Year == "2008" & niotITA_8.2$Origin == "Domestic",]

identical(niotITA_8.4,niotITA_8.5)

View(niotITA_8.5)
```

#### Q.8.6 - Crie um novo conjunto de dados somente com as informações da Demanda Intermediária (i.e faça um subsetting para eliminar as colunas a partir de "Final Consumption Expenditure by Households").


```{r, include=FALSE}
niotITA_8.6 <- niotITA_8.5[,-c(61:67)]
```



#### **Desafio 8.1**: Monte uma matriz de coeficientes técnicos a partir das informaçoes extraídas. Lembrando que os coeficientes são a divisão de um elemento da matriz de demanda intermediária em relação ao setor (que está na coluna).

**Dica**: Usar a matriz criada na questão 8.6, com a demanda intermediária e buscar na base inicial a informação sobre o produto total a preços básicos. Importante: os valores em cada coluna devem ser divididos pelo produto total de um mesmo setor.

```{r, include=FALSE}

total_output <- niotITA_8.2[niotITA_8.2$Year == "2008" & niotITA_8.2$Description == "Output at basic prices",]

total_output_diag <- diag(1/total_output[,5:60])

Dem_Int_matrix <- as.matrix(niotITA_8.6[,5:60])

coef_tec <- Dem_Int_matrix%*%total_output_diag
  
```

#### Q.8.7 - Vamos criar um novo conjunto de dados composto apenas pela soma da Demanda Intemediária, soma da Demanda Final e a Demanda Total. Não esqueça de nomear as colunas.
...
```{r, include=FALSE}
Dem_Int <-rowSums((as.data.frame(niotITA_8.5))[,5:60])

Dem_Fin <-rowSums((as.data.frame(niotITA_8.5))[,61:66])

Dem_Tot <-Dem_Int+Dem_Fin

niotITA_8.7 <- cbind(niotITA_8.5[,1:4],Dem_Int,Dem_Fin,Dem_Tot) 
```
