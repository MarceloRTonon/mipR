...
```
**IMPORTANTE (Nota em 13/09/2024)**
O funcionamento de `&&` e `||` mudou desde que a parte a seguir foi feita. Logo **IGNORE** tudo referente à ela na tabela a seguir:
...
#list(vetor_a && vetor_b,    vetor_a || vetor_b)
```


```{r, eval=F, include=F}
**IGNORE:** Sabemos que nem todos os valores do teste célula à célula para `vetor_a` e `vetor_b` retornam `TRUE`, mas mesmo assim os teste `&&` e `||` retornam `TRUE` pois tem TRUE na primeira célula. Assim sendo, nesse caso acima, não podemos dizer nada sobre a relação do `vetor_a` e `vetor_b` como um todo. No caso de `vetor_c` e `vetor_d`, temos que:
#list(vetor_c && vetor_d,     vetor_c || vetor_d)
```
...
```{r, eval=F, include=F}
**Ignore:** Vamos ver isso agora para reforçar a ideia de `&&` e `||` serem necessária mas não suficiente:
...
## [3.6 - Subsetting com o `tidyverse`](#tidyversesubset)

### 3.6.1 - Extraindo colunas a partir dos seus nomes
...
# Selecionando as colunas 1,3,7 (Se voltar um erro, tente entender o motivo)

selExemplo2 <- mtcars %>% select(c(1,3,7))
pullExemplo2 <- mtcars %>% pull(c(1,3,7))

# Excluindo as colunas 1,3,7 (Se voltar um erro, tente entender o motivo)
...
#### 3.6.2.2 - Removendo duplicatas com `distinc`
...
#### 3.6.2.3 - Reordenando as linhas com `arrange`
...
#### 3.6.2.4 - Removendo linhas `NA` diretamente
...
## [3.7 - Arrumando dados.](#arrumandodadostidyverse)
...
### 3.7.3 - Mudar comprimento de um vetor
...
### 3.7.4 - Alterando tipos de colunas
