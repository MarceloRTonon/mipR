...
Em alguns casos, outra opção é usar dois operadores em sequência `&&` ou `||` que realizaram a mesma tarefa mas pouparão a capacidade computacional do seu computador. Na caso de `&&`, o resultado é automaticamente dado como `FALSE` caso a primeira condição já não seja atendida. Já para `||`, caso a primeira condição seja atendida, o resultado automaticamente é `TRUE`. Uma boa analogia de `||` e `&&` é de `identical` e `==`, que caso você tenha feito os desafios do Tópico 1 você se lembrará que:

<center> **Testando igualdade no R** </center>
...
**É muito importante** que você entenda que `&&` e `||` retornar `TRUE` **NÃO** quer dizer que o teste foi feito para todo o vetor analisado e portanto ele é `TRUE`. Só que o teste para os primeiros valores responderam `TRUE`. Por outro lado, se o resultado retorna `FALSE` você sabe que os vetores não são suficientes.
Para ficar mais claro, vamos considerar os seguintes vetores:
```{r, eval= TRUE}
