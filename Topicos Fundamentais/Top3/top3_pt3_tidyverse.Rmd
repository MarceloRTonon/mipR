...
A função `filter` irá filtrar as linhas a partir de uma condição. Desse modo, vamos usar a base de dados `CO2`^[Para mais informações sobre quaisquer base de dados, use o `help`, como `?CO2` nesse caso.] para exemplificar o caso.
...
`Titanic`^[Registro de mortos e sobreviventes do Titanic. Se ficou interessado dê `?Titanic` para saber mais.] para treinar isso. 
