...
Neste curso não iremos usar o *pipe*, `%>%`, mas explicita-lo ajuda a entender como a escrita orientada ao objeto funciona. No modo Orientado ao objeto, nós temos do lado direito a seguinte estrutura `função_2(função_1(objeto))`, enquanto no modo funcional nós temos `objeto %>% função_1 %>% função_2`. Desse modo, podemos perceber que no caso do modo Orientado ao uma `função()` sempre irá atuar sobre o objeto que esta entre o seu parênteses. Assim sendo, num mesmo comando do script podemos escrever multiplas funções. ***Mudar isso***.
...
Agora que você aprendeu o básico do básico de **R**, podemos então ir para a parte mais voltada para matriz insumo produto!

# Relembrando algebra linear básica.

Vamos relembrar agora alguns conceitos de algebra linear. O primeiro é a multiplicação de matrizes.
