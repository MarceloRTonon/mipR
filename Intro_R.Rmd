---
title: "Apresentação do curso e Introdução ao R"
...
O inicio deste documento se dedicará a explicar os objetivos e motivos curso, bem como o porquê de estarmos utilizando o **R** enquanto linguagem. Se você não se interessa por isto, pode pular para a parte "Onde eu Começo". Se você já tem o **R**, **RStudio** e o **Rtools** (opcional) instalado, podem pular a parte das instalações.
...
A esta altura, se as seções anteriores não foram puladas, você deve estar pensando "ok, você me venceu. Irei programar e usar **R**, só me diz o que eu tenho que fazer! E por favor parae de blábláblá!"
...
**R** é em sua base uma linguagem orientada ao objeto ([Clique aqui](https://pt.wikipedia.org/wiki/Orienta%C3%A7%C3%A3o_a_objetos) para ler mais.). O uso de outros pacotes tal qual `purrr`, que permite que você use dentro do **R** um formato de linguagem funcional. Desse modo **R** se torna uma linguagem multiparadigma. Para baixar um pacote, devemos usar a função `install.packages()`, e para carrega-lo depois, devemos usar a função `library()`.

```{r}
install.packages("purrr") 
library(purrr)

#Modo Orientado ao objeto (usando o R básico):
a <-  sum(c(1:4)) 

#Modo Funcional (usando o R com purrr)
a <- 1:4 %>% c() %>% sum

#Perceba, que podemos escrever 1,2,3,4 como 1:4
```

Neste curso não iremos usar o Modo Funcional, mas explicita-lo ajuda a entender como a escrita orientada ao objeto funciona. No modo Orientado ao objeto, nós temos do lado direito a seguinte estrutura `função_2(função_1(objeto))`, enquanto no modo funcional nós temos `objeto %>% função_1 %>% função_2`. Desse modo, podemos perceber que no caso do modo Orientado ao uma `função()` sempre irá atuar sobre o objeto que esta entre o seu parênteses. Assim sendo, num mesmo comando do script podemos escrever multiplas funções.
...
Fazer suas próprias funções tem ao menos duas vantagens. Em primeiro lugar que se vamos realizar uma mesma série de operações em diferentes momentos do nosso trabalho, estabelecer uma função nos permite poupar tempo e garantir que todas aquelas operações são de fato a mesma (não acabamos escrevendo algumas delas de formas diferentes). A segunda vantagem é podemos que deixar claro no estabelecimento da função como ela funcionará. Repare como quando a função `soma_doida` foi estabelecida, houve um passo a passo do processo, que seria  de certo modo tedioso de ser feito no meio do código. Vale notar que usualmente,
as funções são deixadas no início do código.
...
**Importante:** A depender da situação é necessário usar `[]` ou `[[]]` para acessar elementos de uma lista.
