...
- Os dados referentes a "Oferta de bens e serviços" e a "Produção de atividades" estão juntos. O que os diferencia está na linha 3, de modo que a principio, um data.frame na mesma estrutura que os dados atuais não poderá ser feito se você quiser separar os dois. 
...
1. Os dados da "Oferta de bens e serviços" ficarão juntos com o da produção? 
2. Você faz questão dos nomes da colunas e linhas originais do arquivo?
3. Você quer todos os dados?
4. Quer os dados da soma total?
...
Eu prefiro a solução **a** à solução **b**, uma vez que a primeira permite você manter os nomes originais (menos trabalho), e te permite uma forma mais organizada da base de dados em listas e sublistas. Assim iremos criar o objeto `aba1` que será uma lista e duas sublistas.

```{r}
aba1 <- list("OfertaDeBensServicos"= list(), "ProduçãoDeAtividades" = list())
```

Assim, se quisermos acessar uma base de dados ou outra, basta fazermos `aba1$OfertaDeBensServicos` ou `aba1$ProduçãoDeAtividades`. Você tem assim uma forma de diferenciar e acessar ambas ao mesmo tempo.^[Neste caso, colocamos o nome por extenso, porém você pode escrever uma sigla se você não for esquecer do que se trata.]
