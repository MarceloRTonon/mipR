...
\begin{align} diag(v) = \hat{v} =
\begin{pmatrix} v_{1} & 0 & \dots & 0 \\
0 & v_{2} & \dots & 0 \\
\vdots & \vdots & \ddots & \vdots \\
0 & 0 & \dots & v_n
\end{pmatrix} (\#eq:diagonalmatrix) \end{align}

```{r diag}
diag(a)
```

O **terceiro** caso é quando não se fornece uma matriz ou um vetor atômico de comprimento maior do que 1, mas sim um escalar. Nesse caso a função `diag` irá retornar uma matriz identidade `I_{nxn}` com o valor de `n` sendo igual ao escalar utilizado. \begin{align}
\hat{I} =
\begin{pmatrix} 1 & 0 & \dots & 0 \\
0 & 1 & \dots & 0 \\
\vdots & \vdots & \ddots & \vdots \\
0 & 0 & \dots & 1
\end{pmatrix} (\#eq:diagonalmatrix)
\end{align}
...
\begin{align} \sum_j^m v_{ij} = i'v (\#eq:colSums)
\end{align}
