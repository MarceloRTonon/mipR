...
# Gerando a tabela
formattable(TL,list(
  TL = formatter("span", style = x ~ style(color = ifelse(rank(-x) <= 5, "green", "gray")), x ~ sprintf("%.2f (rank: %02g)", x, rank(-x)))))
...
Antes de prosseguir, iremos replicar a tabela 1 de Morrone (2017), mas antes devemos criar os indicadores ponderados de encadeamentos diretos (que é uma variação do indicador que vimos iniciamente de Chenery e Watanabe) e também o de efeitos diretos e indiretos (Rasmussen), ambos em sua versão de backward linkages. Perceba também que a representação abaixo é equivalente ao cálculo ods indicadores de poder de dispersão discutidos na seção anterior:
$$m=ni'A_n/i'A_ni$$
...
formattable(Indicadores,list(
  C_W = formatter("span", style = x ~ style(color = ifelse(rank(-x) <= 5, "green", "gray")), x ~ sprintf("%.2f (rank: %02g)", x, rank(-x))),
  Rasmussen = formatter("span", style = x ~ style(color = ifelse(rank(-x) <= 5, "green", "gray")), x ~ sprintf("%.2f (rank: %02g)", x, rank(-x))),
  Eigenvector = formatter("span", style = x ~ style(color = ifelse(rank(-x) <= 5, "green", "gray")), x ~ sprintf("%.2f (rank: %02g)", x, rank(-x)))))
