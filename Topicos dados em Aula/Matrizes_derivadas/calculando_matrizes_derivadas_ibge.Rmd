...
$$y = [I-A_{Tot}'] [I-A]^{-1} E_ni $$

```{r}

y <- (diag(nrow(A_Tot)) -t(A_Tot)) %*% solve(diag(nrow(A_n))-A_n) %*% rowSums(E_n)
```


```{r}
Leontief2 <- read_excel(arquivo_2015, sheet = "15", range = "C6:BQ72", col_names = atividades) %>% as.matrix
